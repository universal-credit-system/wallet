				...
				"$dialog_uca")	session_key=`date -u +%Y%m%d`
						...
						then
							...
							then
								...
								then
									now_stamp=`date +%s`
									netcat -q0 -w1 ${uca_ip} ${uca_rcv_port}|gpg --batch --pinentry-mode loopback --output ${user_path}/uca_${now_stamp}.sync --passphrase ${session_key} --decrypt -
									rt_query=$?
									if [ $rt_query = 0 ]
									then
							...
							case $cmd_action in
								...
								"request_uca")	if [ "${cmd_path}" = "" ]
										then
											...
											then
												now=`date +%s`
												netcat -q0 -w1 ${uca_ip} ${uca_rcv_port}|gpg --batch --pinentry-mode loopback --output ${user_path}/uca_${now}.sync --passphrase ${session_key} --decrypt -
												rt_query=$?
												if [ $rt_query = 0 ]
												then
													exit 0
